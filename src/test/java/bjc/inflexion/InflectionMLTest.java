...
public class InflectionMLTest {
	...
	public void testCompiledML() {
		...
		// Check general inflection
		assertCInflects("<#:$1> <N:indexes> $2 found", pair("0 indexes were found", 0, "were"),
				pair("1 index was found", 1, "was"), pair("99 indexes were found", 99, "were"));
		...
		// Check count inflection
		assertCInflects("<#w20:$1> <N:indexes> were found", pair("six indexes were found", 6),
				pair("nineteen indexes were found", 19), pair("20 indexes were found", 20));

		// Check 'n' option
		assertCInflects("<#n:$1> <N:results>", pair("no results", 0), pair("7 results", 7));
		...
		assertCInflects("<#n:$1> <N:item> $2 found", pair("no items were found", 0, "were"));
		assertCInflects("<#ns:$1> <N:item> $2 found", pair("no item was found", 0, "was"));

		// Check article picking
		assertCInflects("<#a:$1> <N:$2>", pair("a result", 1, "results"), pair("3 results", 3, "results"),
				pair("an outcome", 1, "outcomes"), pair("7 outcomes", 7, "outcomes"));

		// Check 'w' option
		assertCInflects("<#w:$1> <N:results>", pair("six results", 6), pair("ten results", 10),
				pair("11 results", 11));
		// Check 'o' option
		assertCInflects("<#o:$1> <N:results>", pair("6th result", 6), pair("11th result", 11),
				pair("22nd result", 22));
		assertCInflects("<#ow:$1> <N:results>", pair("first result", 1), pair("sixth result", 6),
				pair("22nd result", 22));
		assertCInflects("<#o15:$1> <N:results>", pair("6th result", 6), pair("11th result", 11),
				pair("22 results", 22));

		// Check 'f' option
		assertCInflects("Found <#f:$1> <N:matches>", pair("Found no matches", 0), pair("Found one match", 1),
				pair("Found a couple of matches", 2), pair("Found a few matches", 4),
				pair("Found several matches", 8), pair("Found many matches", 11));
		...
		// Check 'e' option
		assertCInflects("Found <#e:$1> <N:matches>", pair("Found no match", 0), pair("Found a match", 1),
				pair("Found ten matches", 10), pair("Found 12 matches", 12));

		// Check 'd' option
		assertCInflects("<#d:$1> <N:Match> found", pair(" Match found", 1), pair(" Matches found", 2));
